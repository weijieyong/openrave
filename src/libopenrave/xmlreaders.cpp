        ...
        case GT_Prism:
            if( xmlname == "points" ) {
                ...
                if( values.size() >= 2 && values.size() % 2 == 0 ) {
                    const size_t nPoints = values.size() / 2;
                    _pgeom->_meshcollision.vertices.reserve(nPoints * 2);
                    _pgeom->_meshcollision.indices.reserve(nPoints * 3 * 2); // 2 * nPoints triangles
                    OpenRAVE::Vector vertex;
                    for( size_t ipoint = 0; ipoint < nPoints; ++ipoint ) {
                        vertex.x = values[ipoint * 2];
                        vertex.y = values[ipoint * 2 + 1];
                        for( dReal z : { -_pgeom->_vGeomData.y * 0.5, _pgeom->_vGeomData.y * 0.5 } ) { // in meter
                            vertex.z = z;
                            _pgeom->_meshcollision.vertices.push_back(vertex);
                        }
                        _pgeom->_meshcollision.indices.push_back(2 * ipoint + 0);
                        _pgeom->_meshcollision.indices.push_back(2 * ipoint + 1);
                        _pgeom->_meshcollision.indices.push_back((2 * ipoint + 2) % (2 * nPoints));
                        _pgeom->_meshcollision.indices.push_back(2 * ipoint + 1);
                        _pgeom->_meshcollision.indices.push_back((2 * ipoint + 3) % (2 * nPoints));
                        _pgeom->_meshcollision.indices.push_back((2 * ipoint + 2) % (2 * nPoints));
                    }
                }
            }
